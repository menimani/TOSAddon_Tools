...
::===== ipf file Copy
if not exist %ExportPath%\ipflist_patched.txt type nul > %ExportPath%\ipflist_patched.txt
xcopy /y /s %TosPath%\patch\*.ipf %TempPath%\patch\*.ipf /EXCLUDE:%ExportPath%\ipflist_patched.txt
cd %TempPath%\patch
dir /on /b > %ExportPath%\ipflist_patch.txt
...
for /f %%a in (ipflist_patch.txt) do (
	...
	::===== Save Pached list
	echo %%a >> %ExportPath%\ipflist_patched.txt
